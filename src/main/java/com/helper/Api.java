...
public class Api {
    ...
    private static Map sign(Map body) {
        ...
        try {
            if (invocable == null) {
                ...
                if (engine == null) {
                    if (!jdk8Warning) {
                        log.warning("请使用jdk1.8版本，高版本不支持请求中的签名参数，不影响功能，只是参数中不带签名");
                        jdk8Warning = true;
                    }
                    return body;
                }
                engine.eval(IoUtil.read(Api.class.getResourceAsStream("sign.js"), UTF_8));
    ...
    private static boolean isSuccess(JSONObject object, String actionName) {
        ...
        if (success == null) {
            if ("405".equals(object.getStr("code"))) {
                log.warning(actionName + "失败:" + "出现此问题有三个可能 1.偶发，无需处理 2.一个账号一天只能下两单  3.不要长时间运行程序，目前已知有人被风控了，暂时未确认风控的因素是ip还是用户或设备相关信息，如果要测试用单次执行模式，并发只能用于6点、8点半的前一分钟，然后执行时间不能超过2分钟，如果买不到就不要再执行程序了，切忌切忌");
                log.warning("405问题解决方案，不保证完全有效,退出App账号重新登录，尝试刷新购物车和提交订单是否正常，如果正常退出小程序重新登录后再抓包，替换UserConfig中的cookie和device_token。");
            } else {
                log.warning(actionName + "失败,服务器返回无法解析的内容:" + JSONUtil.toJsonStr(object));
            }
            return false;
        }
        if (success) {
        ...
        if ("您的访问已过期".equals(object.getStr("message"))) {
            ...
            log.warning("用户信息失效，请确保UserConfig参数准确，并且微信上的叮咚小程序不能退出登录");
            return false;
        }
        String msg = null;
        try {
        ...
        log.warning(actionName + " 失败:" + (msg == null || "".equals(msg) ? "未解析返回数据内容，全字段输出:" + JSONUtil.toJsonStr(object) : msg));
        return false;
    }
    ...
    public static void checkUserConfig() {
        try {
            ...
            log.info("获取可用的收货地址条数：" + validAddress.size());
            for (int i = 0; i < validAddress.size(); i++) {
                ...
                if (address.getBool("is_default")) {
                    ...
                    log.info("获取默认收货地址成功 请仔细核对站点和收货地址信息 站点信息配置错误将导致无法下单");
                    log.info("1.该地址对应城市名称为：" + stationInfo.get("city_name"));
                    log.info("2.该地址对应站点名称为：" + stationInfo.get("name"));
                    log.info("3.该地址详细信息：" + address.getStr("addr_detail") + " 手机号：" + address.getStr("mobile"));
                    log.info("");


//                    if (address.containsValue("city_number") ||  !address.getStr("city_number").equals(com.helper.UserConfig.cityId)) {
...
//                    }
                    if (!stationInfo.getStr("id").equals(UserConfig.get("ddmc-station-id"))) {
                        UserConfig.set("ddmc-station-id", stationInfo.getStr("id"));
                        log.info("站点id配置不正确，已自动填入获取到的id");
                    } else {
                        log.info("站点id配置正确");
                    }
                    if (!address.getStr("id").equals(UserConfig.get("ddmc-address-id"))) {
                        UserConfig.set("ddmc-address-id", address.getStr("id"));
                        log.info("地址id配置不正确，已自动填入获取到的id");
                    } else {
                        log.info("地址id配置正确");
                    }
                    return;
        ...
        log.warning("没有可用的默认收货地址，请自行登录叮咚设置该站点可用的默认收货地址");
    }
    ...
    public static void allCheck() {
        try {
            ...
            httpRequest.addHeaders(UserConfig.getHeaders());
            Map<String, String> request = UserConfig.getParamsBody();
    ...
    public static Map<String, Object> getCart(boolean noProductsContinue) {
        try {
            ...
            log.info("更新购物车数据成功,订单金额：" + newOrderProduct.get("total_money"));
            return map;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static Map<String, Object> getMultiReserveTime(String addressId, Map<String, Object> cartMap) {
        try {
            ...
            Map<String, String> headers = UserConfig.getHeaders();
            httpRequest.addHeaders(headers);
            Map<String, Object> request = UserConfig.getFormBody();
    ...
    public static boolean addNewOrder(String addressId, Map<String, Object> cartMap, Map<String, Object> multiReserveTimeMap, Map<String, Object> checkOrderMap) {
        try {
            ...
            context.put("end", new HashMap<>());
            for (int i = 0; i < 10; i++) {
                log.info("恭喜你，已成功下单 当前下单总金额：" + cartMap.get("total_money"));
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
