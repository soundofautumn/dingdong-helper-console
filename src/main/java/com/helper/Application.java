...
public class Application {
    ...
    private static boolean timeTrigger(int hour, int minute, int second) {
        ...
        log.info("时间触发 当前时间 " + currentHour + ":" + currentMinute + ":" + currentSecond + " 目标时间 " + hour + ":" + minute + ":" + second);
                ...
                while (!Api.context.containsKey("end")) {
                    ...
                    if (cartMap != null) {
                        if (Double.parseDouble(cartMap.get("total_money").toString()) < minOrderPrice) {
                            log.warning("订单金额：" + cartMap.get("total_money").toString() + " 不满足最小金额设置：" + minOrderPrice + " 继续重试");
                        } else {
        ...
        for (int i = 0; i < baseTheadSize; i++) {
            new Thread(() -> {
                while (!Api.context.containsKey("end")) {
                    ...
                    Map<String, Object> multiReserveTimeMap = Api.getMultiReserveTime(UserConfig.get("ddmc-address-id"), Api.context.get("cartMap"));
        ...
        for (int i = 0; i < baseTheadSize; i++) {
            new Thread(() -> {
                while (!Api.context.containsKey("end")) {
                    ...
                    Map<String, Object> checkOrderMap = Api.getCheckOrder(UserConfig.get("ddmc-address-id"), Api.context.get("cartMap"), Api.context.get("multiReserveTimeMap"));
        ...
        for (int i = 0; i < submitOrderTheadSize; i++) {
            new Thread(() -> {
                while (!Api.context.containsKey("end")) {
                    ...
                    if (Api.addNewOrder(UserConfig.get("ddmc-address-id"), Api.context.get("cartMap"), Api.context.get("multiReserveTimeMap"), Api.context.get("checkOrderMap"))) {
                        log.info("铃声持续1分钟，终止程序即可，如果还需要下单再继续运行程序");
                        Api.play();
                    }
                }
            }).start();
